...
for i in range(numFile):
  x = sqlContext.read.format('com.databricks.spark.csv').options(header='true',inferschema='true').load('flight/'+str(1987+i)+'.csv')
  ...
  xtype = x.dtypes
   for t in xtype:
     if t[1]=='string' and t[0] not in ['Origin','Dest','TailNum','UniqueCarrier','FlightNum']
       x=x.withColumn(t[0],new_column_udf(x[t[0]]))
